#!/usr/bin/env python3
"""
fixed_cuda_bkm_v2_unified.py - Definitive Unified Version
===========================================================
CUDA-accelerated BKM integral solver with all critical fixes and features.

This is the SINGLE DEFINITIVE VERSION to use across all tests.

Version History Consolidated:
- v2 base: De-aliasing, skew-symmetric convection, alignment metrics
- v2 corrected: Emergency brake bug fix (respects rho_hard)
- v3 additions: 3D safety checks, compute_slices method
- Unified: All fixes + clean separation (no test-specific code)
- Final: Added provenance stamp and enhanced safety checks
...
- Engine provenance tracking
"""

# Engine provenance stamp for traceability
ENGINE_ID = "fixed_cuda_bkm_v2_unified@2025-01-23"
...
parser = argparse.ArgumentParser(description="Unified BKM Engine")
...
class CUDABKMSolver:
    """
    Production-ready CUDA-accelerated Navier-Stokes solver with BKM integral tracking
    Unified version with all critical fixes and features
    """
    ...
    def __init__(self, grid_size=64, dt=0.001, reynolds_number=1000.0,
        ...
        """
        Initialize solver with production-ready defaults
        
        Parameters include all v2/v3 features plus dtype/precision support
        """
        ...
        if dtype is not None:
            ...
            self.dtype = xp.float64  # Default to float64
        ...
        self.kx = kx.reshape(-1, 1, 1).astype(self.dtype)
        self.ky = ky.reshape(1, -1, 1).astype(self.dtype)
        self.kz = kz.reshape(1, 1, -1).astype(self.dtype)
        ...
        # Pre-compute i*k for in-place derivative operations (memory optimization)
        complex_dtype = self.xp.complex64 if self.dtype == self.xp.float32 else self.xp.complex128
        self.ikx = (1j * self.kx).astype(complex_dtype)
        self.iky = (1j * self.ky).astype(complex_dtype)
        self.ikz = (1j * self.kz).astype(complex_dtype)
        ...
        self.u = self.xp.zeros((self.nx, self.ny, self.nz), dtype=self.dtype)
        self.v = self.xp.zeros((self.nx, self.ny, self.nz), dtype=self.dtype)
        self.w = self.xp.zeros((self.nx, self.ny, self.nz), dtype=self.dtype)
        ...
        print(f"Solver initialized (unified version): Re={reynolds_number}, CFL={CFL_target}")
    ...
    def compute_derivatives_fft(self, field, axis):
        """
        Spectral derivatives with defensive 3D check and proper de-aliasing
    ...
    def compute_slices(self, axis='z', index='mid', include_diagnostics=True):
        ...
        # Extract velocity slices
        u_slice = self.u[slicer].astype(xp.float64)
        v_slice = self.v[slicer].astype(xp.float64)
        w_slice = self.w[slicer].astype(xp.float64)
        ...
        if include_diagnostics:
            ...
            # Extract vorticity slices
            omega_x_slice = omega_x[slicer].astype(xp.float64)
            omega_y_slice = omega_y[slicer].astype(xp.float64)
            omega_z_slice = omega_z[slicer].astype(xp.float64)
    ...
    def compute_slices_simple(self):
        ...
        return {
            "xy": speed[:, :, zmid].astype(xp.float64),
            "xz": speed[:, ymid, :].astype(xp.float64),
            "yz": speed[xmid, :, :].astype(xp.float64),
        }
    ...
    def compute_alignment_metrics(self, u, v, w):
        ...
        # Compute alignment metric: cos(theta) = |omega . (S*omega)| / (|omega| * |S*omega|)
    ...
    def dealias_field(self, field):
        ...
        if self.use_dealiasing:
            field_hat *= self.dealias_mask
        return self.fft.ifftn(field_hat).real

    def umax(self):
        """Measure current max |u| from de-aliased fields"""
        # Use de-aliased fields for measurement
    ...
    def compute_laplacian_fft(self, field):
        """Spectral Laplacian with optional de-aliasing"""
        field_hat = self.fft.fftn(field)
        if self.use_dealiasing:
            field_hat *= self.dealias_mask
        laplacian_hat = -self.k2 * field_hat
        return self.fft.ifftn(laplacian_hat).real
    ...
    def compute_divergence_metrics_fields(self, u, v, w):
        """Compute divergence metrics for given fields"""
    ...
    def compute_max_vorticity(self):
        """Compute ||omega||_inf - the maximum vorticity magnitude"""
        omega_x = self.compute_derivatives_fft(self.w, 1) - self.compute_derivatives_fft(self.v, 2)
        omega_y = self.compute_derivatives_fft(self.u, 2) - self.compute_derivatives_fft(self.w, 0)
        omega_z = self.compute_derivatives_fft(self.v, 0) - self.compute_derivatives_fft(self.u, 1)
    ...
    def compute_energy_spectrum(self):
        ...
        uhat = self.fft.fftn(self.u)
        vhat = self.fft.fftn(self.v)
        what = self.fft.fftn(self.w)
    ...
    def load_checkpoint(self, filename):
        ...
        try:
            ...
            with h5py.File(filename, 'r') as f:
                ...
                # Convert to CuPy if using GPU
                if self.use_gpu:
                    self.u = self.xp.asarray(u_load)
                    self.v = self.xp.asarray(v_load)
                    self.w = self.xp.asarray(w_load)
                else:
                    self.u = u_load
                    self.v = v_load
                    self.w = w_load
...
if __name__ == "__main__":
    print("Testing CUDA BKM Engine...")
    print(f"Backend detected: {backend}")
    
    # Try to create a small solver instance
    solver = CUDABKMSolver(grid_size=32, reynolds_number=100)
